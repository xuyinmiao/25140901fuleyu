...
- `--llm mock`：离线可复现，用固定决策模拟 Tool Calling。
- `--llm deepseek`：使用 DeepSeek OpenAI-compatible API，读取 `DEEPSEEK_API_KEY`。
- `--llm openai`：使用 OpenAI API，读取 `OPENAI_API_KEY`。
...
若没有可用 API key，可使用 `--llm mock` 生成稳定 ReAct 决策；真实路径探索、状态约束和输入求解均由 angr 执行。若需要真实 OpenAI 工具调用日志，可设置 `OPENAI_API_KEY` 和 `OPENAI_MODEL` 后运行：
